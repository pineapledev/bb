...
bool handle_input(JoyShock *jc, uint8_t *packet, int len, bool &hasIMU) {
	...
	jc->last_ds3_state = jc->ds3_state;
	...
	if (jc->controller_type == ControllerType::s_ds3) {
        // printf("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
        //     packet[0], packet[1], packet[2], packet[3], packet[4], packet[5], packet[6], packet[7], packet[8], packet[9],
        //     packet[10], packet[11], packet[12], packet[13], packet[14], packet[15], packet[16], packet[17], packet[18], packet[19], packet[20],
        //     packet[21], packet[22], packet[23], packet[24], packet[25], packet[26], packet[27], packet[28], packet[29], packet[30],
...
#ifdef _WIN32
		if (packet[0] != DS3_FEATURE_REPORT_ID) return false;
#else
		if (packet[0] != 0x01 || packet[1] == 0xFF) return false;
#endif

		ds3_input_report report;
		std::memcpy(&report, &packet[DS3_HID_OFFSET], sizeof(ds3_input_report));

		/* jc->battery is not really used so uuuuh
		if (report.battery_status >= 0xEE)
		{
		...
		{
			jc->battery = ::at32(battery_capacity, std::min<usz>(report.battery_status, battery_capacity_size - 1));
		}
		*/
        ...
        int16_t gyroSampleY = uint16_to_int16(report.gyro[0] << 8 | (report.gyro[1]) & 0xFF00);
		int16_t accelSampleX = uint16_to_int16(report.accel_x[0] << 8 | (report.accel_x[1]) & 0xFF00);
		int16_t accelSampleY = uint16_to_int16(report.accel_y[0] << 8 | (report.accel_y[1]) & 0xFF00);
		int16_t accelSampleZ = uint16_to_int16(report.accel_z[0] << 8 | (report.accel_z[1]) & 0xFF00);

        if ((gyroSampleY | accelSampleX | accelSampleY | accelSampleZ) == 0) {
            // all zero?
            hasIMU = false;
        }

        // convert to real units
        // i dont understand the gyro
		//imu_state.gyroY = (float) (gyroSampleY);

#ifdef _WIN32
		// Official Sony Windows DS3 driver seems to do the same modification of this value as the ps3
		imu_state.accelX  = accelSampleX;
#else
		// When getting raw values from the device this adjustement is needed
		imu_state.accelX  = 512 - (static_cast<u16>(accelSampleX) - 512);
#endif
        imu_state.accelX = (float) (accelSampleX - 512) / 27819.3210557f;
        imu_state.accelY = (float) (accelSampleY - 512) / 27819.3210557f;
        imu_state.accelZ = (float) (accelSampleZ - 512) / 27819.3210557f;

		//imu_state.gyroY = (float) (gyroSampleY) / (123.f / 90.f);
        //printf("DS accel: %.4f, %.4f, %.4f\n", imu_state.accelX, imu_state.accelY, imu_state.accelZ);

        //printf("%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%d\n",
        //	jc->gyro.yaw, jc->gyro.pitch, jc->gyro.roll, jc->accel.x, jc->accel.y, jc->accel.z, universal_counter++);
		// 0
		// l dpad, u = 1, r = 2, d = 4, l = 8
		// r select = 1, l3 = 2, r3 = 4, start = 8
		// 1
		// l triangle = 1, circle = 2, cross = 4, square = 8
		// r l2 = 1, r2 = 2, l1 = 4, r1 = 8
		// 2
		// r home = 1
		// 4 LSx, 5 LSy, 6 RSx, 7 RSy. x 00 to ff left to right, y 00 to ff up to down.
		// 12 dpad up, 13 dpad right, 14 dpad down, 15 dpad left
		// 16 l2, 17 r2, 18 l1, 19 r1
		// 20 triangle, 21 circle, 22 cross, 23 square
		//


        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 6) << JSMASK_DOWN) & JSMASK_DOWN; // down = SE | S | SW
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 4) << JSMASK_UP) & JSMASK_UP; // up = N | NE | NW
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 5) << JSMASK_RIGHT) & JSMASK_RIGHT; // right = NE | E | SE
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 7) << JSMASK_LEFT) & JSMASK_LEFT; // left = SW | W | NW

        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 7) << JSOFFSET_W) & JSMASK_W;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 6) << JSOFFSET_S) & JSMASK_S;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 5) << JSOFFSET_E) & JSMASK_E;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 4) << JSOFFSET_N) & JSMASK_N;
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 1) << JSOFFSET_LCLICK) & JSMASK_LCLICK;
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 2) << JSOFFSET_RCLICK) & JSMASK_RCLICK;
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 3) << JSOFFSET_PLUS) & JSMASK_PLUS;
        jc->simple_state.buttons |= ((int) (report.buttons[0] >> 0) << JSOFFSET_MINUS) & JSMASK_MINUS;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 3) << JSOFFSET_R) & JSMASK_R;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 2) << JSOFFSET_L) & JSMASK_L;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 1) << JSOFFSET_ZR) & JSMASK_ZR;
        jc->simple_state.buttons |= ((int) (report.buttons[1] >> 0) << JSOFFSET_ZL) & JSMASK_ZL;
        jc->simple_state.buttons |= ((int) (report.buttons[2] >> 0) << JSOFFSET_PS) & JSMASK_PS;
        //jc->btns.zr = (packet[indexOffset+6] >> 3) & 1;
        //jc->btns.zl = (packet[indexOffset+6] >> 2) & 1;
        jc->simple_state.rTrigger = static_cast<float>(report.button_values[5]) / 255.0f;
        jc->simple_state.lTrigger = static_cast<float>(report.button_values[4]) / 255.0f;

        uint16_t stick_x = report.lsx;
        uint16_t stick_y = report.lsy;

		stick_y = 255 - stick_y;

		uint16_t stick2_x = report.rsx;
		uint16_t stick2_y = report.rsy;
		stick2_y = 255 - stick2_y;
        ...
        jc->ds3_state.dpadUp = static_cast<float>(report.button_values[0]) / 255.0f;
        jc->ds3_state.dpadRight = static_cast<float>(report.button_values[1]) / 255.0f;
        jc->ds3_state.dpadDown = static_cast<float>(report.button_values[2]) / 255.0f;
        jc->ds3_state.dpadLeft = static_cast<float>(report.button_values[3]) / 255.0f;
        jc->ds3_state.l1 = static_cast<float>(report.button_values[6]) / 255.0f;
        jc->ds3_state.r1 = static_cast<float>(report.button_values[7]) / 255.0f;
        jc->ds3_state.triangle = static_cast<float>(report.button_values[8]) / 255.0f;
        jc->ds3_state.circle = static_cast<float>(report.button_values[9]) / 255.0f;
        jc->ds3_state.cross = static_cast<float>(report.button_values[10]) / 255.0f;
        jc->ds3_state.square = static_cast<float>(report.button_values[11]) / 255.0f;

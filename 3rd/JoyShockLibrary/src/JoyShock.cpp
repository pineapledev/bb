...
#define DS3_FEATURE_REPORT_ID 0xF2
#ifdef _WIN32
...
constexpr u8 battery_capacity_size = 6;
constexpr u8 battery_capacity[battery_capacity_size] = {0, 1, 25, 50, 75, 100};
...
public:
	void init(struct hid_device_info *dev, hid_device* inHandle, int uniqueHandle, const std::string &inPath) {
		...
		if (dev->product_id == DS3_USB) {
			this->name = std::string("DualShock 3");
			this->left_right = 3; // left and right?
			this->controller_type = ControllerType::s_ds3;
			this->is_usb = true;
		}
...
*/
	...
	void set_ds3_rumble_light(bool smallRumble, unsigned char bigRumble, unsigned char playerLights) {
		if (!is_usb) {
			// teehee set_ds3_rumble_bt(smallRumble, bigRumble, playerLights);
		}
		else {
			set_ds3_rumble_usb(smallRumble, bigRumble, playerLights);
		}
	}

    void set_ds3_rumble_usb(bool smallRumble, unsigned char bigRumble, unsigned char playerLights)
    {
		ds3_output_report outputReport{};
		outputReport.rumble.small_motor_duration = 0xfe;
		outputReport.rumble.small_motor_on = smallRumble;
		outputReport.rumble.large_motor_duration = 0xfe;
		outputReport.rumble.large_motor_force = bigRumble;

		outputReport.led_enabled = playerLights;

        hid_write(handle, &outputReport.report_id, sizeof(ds3_output_report));
    }

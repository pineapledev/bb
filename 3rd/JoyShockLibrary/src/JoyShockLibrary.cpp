...
void pollIndividualLoop(JoyShock *jc) {
	...
	while (!jc->cancel_thread) {
		...
		if (jc->controller_type == ControllerType::s_ds3)
		{
			buf[0] = 0xF2;
#ifdef _WIN32
			res = hid_get_feature_report(jc->handle, buf, 64);
#else
			res = hid_read_timeout(jc->handle, buf, 64, 1000);
#endif
		}
		else
		{
		...
		else
		{
			...
			if (handle_input(jc, buf, 64, hasIMU)) { // but the user won't necessarily have a callback at all, so we'll skip the lock altogether in that case
				...
				if (_pollCallback != nullptr || _pollTouchCallback != nullptr)
				{
					...
					if (jc->controller_type != ControllerType::n_switch && jc->controller_type != ControllerType::s_ds3 && _pollTouchCallback != nullptr) {
...
int JslConnectDevices()
{
	...
	while (cur_dev) {
		...
		if (handle != nullptr)
		{
			...
			if (currentJc != nullptr)
			{
				...
				else if (currentJc->controller_type == ControllerType::s_ds3) {
					if (currentJc->is_usb) {
						currentJc->deinit_ds3_usb();
					}
					else {
						currentJc->deinit_ds3_bt();
					}
				}
		...
		if(cur_dev->next && *cur_dev->serial_number == *cur_dev->next->serial_number)
		{
			cur_dev = cur_dev->next;
		}
		cur_dev = cur_dev->next;
	...
	for (std::pair<int, JoyShock*> pair : _joyshocks)
	{
		...
		else if (jc->controller_type == ControllerType::s_ds3)
		{
			jc->initialised = true;
		} // charging grip
		else if (jc->is_usb) {
...
DS3_STATE JslGetDS3State(int deviceId)
{
	std::shared_lock<std::shared_timed_mutex> lock(_connectedLock);
	JoyShock* jc = GetJoyShockFromHandle(deviceId);
	if (jc != nullptr) {
...
JSL_SETTINGS JslGetControllerInfoAndSettings(int deviceId)
{
	...
	if (jc != nullptr) {
		...
		case ControllerType::s_ds3:
			settings.controllerType = JS_TYPE_DS3;
			break;
		default:
		case ControllerType::n_switch:
...
void JslSetRumble(int deviceId, int smallRumble, int bigRumble)
{
    ...
    else if (jc != nullptr && jc->controller_type == ControllerType::s_ds3) {
		jc->modifying_lock.lock();
        jc->small_rumble = smallRumble;
        jc->big_rumble = bigRumble;
        jc->set_ds3_rumble_light(
                jc->small_rumble,
                jc->big_rumble,
                jc->player_number);
		jc->modifying_lock.unlock();
